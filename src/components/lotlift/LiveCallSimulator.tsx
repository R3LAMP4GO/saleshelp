...
type Turn = { id: number; text: string; response: { id: string; title: string; response: string; consideration: string; rule_id: string; source: string } | null };
...
export function LiveCallSimulator() {
  ...
  async function addProspectLine(text: string) {
    ...
    const conversation = [...turnsRef.current.map((turn) => ({ ...segment, id: `sim-${turn.id}`, text: turn.text })), segment];
    ...
    const fallbackMove = selectLotLiftNextMove({ state: callStateRef.current, turn: segment, conversation });
    if (coaching) {
    ...
    } else applyStateEvents(fallbackMove.state_events);
    ...
    appendTurn({ id, text: cleaned, response });
    ...
    if (mode !== "local-ai" || fallbackMove.source !== "approved-move") return;
    const controller = new AbortController();
    localSelectionAbortRef.current = controller;
    try {
      const result = await analyzeLotLiftTurn({ state: callStateRef.current, turn: segment, conversation, settings, signal: controller.signal });
      ...
      if (result.source === "model" && result.selected_move && result.spoken_response) {
        ...
        const selected = result.selected_move;
        const updated = { id: selected.id, title: selected.title, response: selected.response, consideration: selected.goal, rule_id: selected.id, source: "safe-fallback approved response" };
        turnsRef.current = turnsRef.current.map((turn) => turn.id === id ? { ...turn, response: updated } : turn);
        setTurns(turnsRef.current);
  ...
  function loadExample() {
    ...
    for (const [index, text] of EXAMPLE_TURNS.entries()) {
      ...
      exampleTurns.push({ id: index, text, response: coaching
        ? { ...coaching.response, source: "approved-card" }
        : { id: fallbackMove.id, title: fallbackMove.title, response: fallbackMove.response, consideration: fallbackMove.goal, rule_id: fallbackMove.id, source: fallbackMove.source } });
    }
        ...
        <div className="grid min-h-0 gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
          ...
          <aside className="rounded-lg border bg-card" aria-labelledby="response-heading" aria-live="polite">
            ...
            <div className="space-y-5 p-4 sm:p-5">
              {latest ? <>
                <div><h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Say this now</h3><p className="mt-2 text-lg font-medium leading-7">{latest.response}</p></div>
