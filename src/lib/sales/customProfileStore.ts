...
import { type CustomSalesProfile, validateCustomSalesProfile } from "./customProfiles";
...
function parseProfiles(raw: string): CustomSalesProfile[] {
  ...
  return parsed.map((profile) => validateCustomSalesProfile(profile as CustomSalesProfile));
}
...
export async function pickSalesPlaybook(): Promise<ImportedSalesPlaybook | null> {
  ...
  const selected = await open({
    ...
    filters: [{ name: "Playbook", extensions: ["md", "markdown", "txt", "pdf"] }],
  });
